...
async function signed(token) {
  ...
  try {
    ...
    return JSON.parse(atob(base64 + "=".repeat((4 - (base64.length % 4)) % 4))).exp > Date.now();
  } catch { return false; }
}

export async function middleware(request) {
  if (await signed(request.cookies.get("invoice_session")?.value)) return NextResponse.next();
...
export const config = { matcher: ["/dashboard", "/invoices/:path*", "/api/invoices/:path*"] };
